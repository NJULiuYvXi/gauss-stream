...
当前桌面版本：`0.3.8`。Windows 便携程序随同名 GitHub Release 发布。

## 版本记录

### 0.3.8

- 滚轮和触控缩放改为指向位置聚焦，不再只能朝模型总中心缩放。
- 最小轨道距离由场景对角线的 1/10000 降低到 1/10000000，可进入局部建筑和高斯细节尺度。
- 相机近裁剪面改为随当前焦点距离动态调整，近距离观察时不会被旧的固定裁剪下限挡住。
- 保留模型中心轨道旋转；缩放到局部后，旋转焦点会自然转移到正在观察的区域。

### 0.3.7

- 相机交互改为具有明确 target 的轨道控制，旋转、缩放和触控始终围绕模型中心，不再绕相机自身旋转。
- 修正 Spark 分页根块的优先级坐标：每个 tile 使用自己的世界空间包围盒中心，不再错误地共用模型对象原点。
- 根块加载顺序改为当前视野优先、屏幕中心向边缘扩展，同一视线内再由近到远；视野外预加载 tile 排在当前正面之后。
- 这里的“正面”明确指当前相机所面对的可见侧，而不是无法从无符号高斯协方差中推断出的建筑入口等语义正面。
